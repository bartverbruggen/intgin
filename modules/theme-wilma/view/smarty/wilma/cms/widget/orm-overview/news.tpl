...
<div class="block block--panel {$app.cms.properties->getWidgetProperty('style.container')}" id="widget-{$app.cms.widget}">
    ...
    {if $result}
        {foreach from=$result item="content"}
            ...
            *}
            {call renderOverviewBlock class=$excerptClass image=$content->image title=$content->title url=$content->url teaser=$content->teaser date=$content->data->getDateAdded()}
