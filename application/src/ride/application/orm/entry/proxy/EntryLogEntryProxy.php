...
/**
 * Generated proxy for an entry of the EntryLog model
 * 
 * NOTE: Do not edit this class
 */
class EntryLogEntryProxy extends EntryLogEntry implements EntryProxy {

    /**
     * Instance of the EntryLog model
    ...
    /**
     * Construct a new EntryLog entry proxy
     * @param \ride\library\orm\model\Model $model Instance of the EntryLog model
    ...
    /**
     * Loads the values of the properties of this EntryLog entry
     * @return null
     */
    private function loadProperties() {
        ...
        if (!isset($this->loadedFields['model'])) {
            $this->model = $entry->getModel();
            $this->loadedValues['model'] = $entry->loadedValues['model'];
            $this->loadedFields['model'] = true;
        }
        ...
        if (!isset($this->loadedFields['action'])) {
            $this->action = $entry->getAction();
            $this->loadedValues['action'] = $entry->loadedValues['action'];
            $this->loadedFields['action'] = true;
        }
    ...
    /**
     * Loads the value of a relation field of this EntryLog entry
    ...
    public function setModel($model) {
        $hasOldValue = false;
        $oldValue = null;
        if (array_key_exists('model', $this->loadedValues)) {
            $oldValue = $this->loadedValues['model'];
            $hasOldValue = true;
        } elseif ($this->id && !isset($this->loadedFields['model'])) {
            $oldValue = $this->getModel();
            $hasOldValue = true;
        }
        if ($hasOldValue && $oldValue === $model)  {
            return;
        }
    ...
    public function setAction($action) {
        $hasOldValue = false;
        $oldValue = null;
        if (array_key_exists('action', $this->loadedValues)) {
            $oldValue = $this->loadedValues['action'];
            $hasOldValue = true;
        } elseif ($this->id && !isset($this->loadedFields['action'])) {
            $oldValue = $this->getAction();
            $hasOldValue = true;
        }
        if ($hasOldValue && $oldValue === $action)  {
            return;
        }
    ...
    public function setChanges(array $changes = array()) {
        $this->loadedFields['changes'] = true;
    ...
    public function getChanges() {
        if (!isset($this->loadedFields['changes'])) {
            $this->loadRelation('changes');
        }
